...
export default function MatchStatRow(props) {
  ...
  const [matchStatRow, setMatchStatRow] = useState({
    id: props.id,
    playerId: players[0] ? players[0].id : "",
    contactType: CONTACT_TYPES[0],
    qualityType: QUALITY_TYPES[0],
  });
  ...
  CONTACT_TYPES.forEach((contactType) =>
    contactTypeOptions.push(<option value={contactType}>{contactType}</option>)
  );
  QUALITY_TYPES.forEach((qualityType) =>
    qualityTypeOptions.push(<option value={qualityType}>{qualityType}</option>)
  );
  ...
  return (
    <tr className={rowStyles.join(" ")}>
      ...
      <td>
        <select
          onInput={(e) => selectHandler(e, "playerId")}
      ...
      <td>
        <select
          onInput={(e) => selectHandler(e, "contactType")}
      ...
      <td>
        <select
          onInput={(e) => selectHandler(e, "qualityType")}
      ...
      <td>
        <input
          checked={matchStatRow.pointScored}
          type="checkbox"
          onChange={pointUpdateHandler}
        />
      </td>
      <td>
        <FontAwesomeIcon
