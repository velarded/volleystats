...
// Matches Collection
export const addNewMatchSet = async (uid, newMatchSet) => {
    const docRef = await addDoc(collection(db, 'users', uid,'matchSets'), newMatchSet).catch(e => console.log('Error with adding new match set: ', e));
    console.log("Document written with ID: ", docRef);
    return docRef.id;
};
...
export const addNewPlayerHighlight = async(uid, newPlayerHighlightData) => {
    const docRef = await addDoc(collection(db, 'users', uid, 'highlights'), newPlayerHighlightData).catch(e => console.log('Error with adding new player highlight: ', e));
    console.log("Document written with ID: ", docRef);
    return docRef.id;
};
